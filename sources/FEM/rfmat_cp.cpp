	  ...
	  /*  De is calculated independently from element porosity. We use node porosity values with with Archies law De = Dp * poros^m   and do a harmonic average of the node diffusion coefficients!   as Dp is part of the dispersion tensor, we  modify Dp -> Dp=Dp0*poro^(m-1)*/
	   ...
	   // now do the thing with  
        ...
        case 30: // diffusion coeffcient is scaled to relative porosity change including a critical porosity value
        {
            ...
            // no saturation coupling implemented yet
            flowflag = m_vec_GEM->REACT_GEM::GetFlowType_MT();
            if ( flowflag == 3)  // RICHARDS_FLOW
            {
                throw std::runtime_error("advection not supported for case 30 currently");
            }
            ...
            for (i = 0; i < count_nodes; i++) //calculate harmonic mean of node based diffusion coefficients
            {
                ...
                if(nodePoros > porosCrit)
                {
                    ...
                    // diffusion coeffcient shall never be lower than minumum De
                    De = std::max(De, De_min);
            ...
            // correct for multiplication with element porosities -> Pore diffusion coefficient
            Dm = De / porosity;
